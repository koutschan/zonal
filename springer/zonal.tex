...
\keywords{zonal polynomial\and symmetric function\and integer partition\and Laplace-Beltrami operator\and
Wishart matrix\and hypergeometric function of a matrix argument}
 \subclass{Primary 05E05 \and Secondary 33C20\and 33C70\and 15B52\and 65C60}
\end{abstract}
...
\begin{rem}
As mentioned in Remark 1 on page 73 of  \cite{Takemura}, recurrence \eqref{eq:CRec} is not complete until $\rho_\kappa-\rho_\lambda\neq 0$. However, this is not true. The first example is when $n=6$, $\kappa=(4,1,1)$, and $\lambda=(3,3)$, in which 
\[
\rho_\kappa=4\cdot(4-1)+1\cdot(1-2)+1\cdot(1-3)=9=3\cdot(3-1)+3\cdot(3-2)=\rho_\lambda.
\]
Observation shows that, not only does the denominator in \eqref{eq:CRec} vanish, but also the summation is empty. 
It seems that James \cite{James1} has claimed that for all ``relevant'' pairs $(\kappa,\lambda)$, $c_{\kappa,\lambda}>0$. Due to the no nnegative numerator of the summation in \eqref{eq:CRec}, it indicates that if $\rho_\kappa\leq\rho_\lambda$,  $c_{\kappa,\lambda}=0$. 

See the example at the end of Section \ref{sec:Package} of the case $n=6$, $\kappa=(4,1,1)$, and $\lambda=(3,3)$, which is also compatible with the build-in package in SageMath.
\end{rem}
...
=&a_{m}\sum_{i=1}^{m}(\kappa_{i}-\lambda_{i})+(a_{m-1}-a_{m})\sum_{i=1}^{m-1}(\kappa_{i}-\lambda_{i})+\cdots\\
  & +(a_{1}-a_{2})(\kappa_{1}-\lambda_{1})\geq 0.
\end{align*}
