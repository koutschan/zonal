...
\begin{itemize}
\item $\Lambda_{\nu}=\diag\left(2^{n}\left(\nu/2\right)_{\!\lambda}\right)$ for for $\lambda\in\mathcal{P}_n$ is the diagonalization of $T_\nu$; 
...
\right)^T,
\end{equation}
and define $\mathcal{C}_{\lambda}(Y)=d_{\lambda}\mathcal{Y}_{\lambda}(Y)$ by some constants $d_\lambda$, given by
\[
...
\end{thm}
\begin{proof}
Obviously, $\rho_{\left(n\right)}=n\left(n-1\right)$ and $\rho_{\left(n-m,m\right)}=\left(n-m\right)\left(n-m-1\right)+m\left(m-2\right)$,
so that 
\[%begin{equation}
...
\end{align*}
Now, we proceed by induction on~$m$. When $m=1$, $t=1$ in the sum above is equivalen to $\mu=(n)$, so that
...
\[
 c_{\left(n-2,2\right),\left(n-2,2\right)}=\binom{n}{n-2,2}-c_{\left(n\right),\left(n-2,2\right)}-c_{\left(n-1,1\right),\left(n-2,2\right)}=\frac{2n\left(n-1\right)\left(n-2\right)\left(n-3\right)}{\left(2n-3\right)\left(2n-5\right)}.
\]
\qedhere
%\begin{align*}
...
%  \end{align*}
  ...
  parts of~$\kappa$ (, noting that $\kappa_4=0$).
\end{conj}
