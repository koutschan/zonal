...
\begin{abstract}
  We investigate zonal polynomials, a family of symmetric polynomials that
  ...
  computation. Based on them, we carry our an experimental mathematics study
...
generalized to matrix arguments, as follows:
\begin{defn}
...
\]
\item the Pochhammer symbol, defined by, for positive integer $m$,  that $\left(a\right)_{m}:=a\left(a+1\right)\cdots\left(a+m-1\right)$
is generalized to define
\[
...
unterstand the hypergeometric function at matrix argument is the zonal
...
\end{rem}
\begin{prop}
Given $X\in \SPD(m)$, James \cite[eq.~3.12, p.~1712]{James1} derived
...
\]
where $X=HYH^{T}$, for $H\in O(n)$, the $n$th orthogonal group, and $Y=\diag(y_{1},\ldots,y_{n})$ is the diagonalization of $X$. 
\end{prop}
\begin{rem}
