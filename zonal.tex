...
\keywords{zonal polynomial, symmetric function, integer partition, Laplace-Beltrami operator,
  Wishart matrix, hypergeometric function of a matrix argument} % TODO more
...
From Definition~\ref{eq:MatrixpFq} we see that the key to understand the
hypergeometric function with matrix argument is the zonal
polynomial~\cite[\S\,35.4]{DLMF}. For evaluating the hypergeometric
...
accurate initial conditions. A comprehensive introduction to zonal
...
Good references for the material in this subsection are~\cite{James1,Moakher}.
...
the projection
\begin{equation}
\left(\Tr Y\right)^{n}\Big|_{V_{\lambda}}=\mathcal{C}_{\lambda}(Y)\label{eq:projection}
\end{equation}
...
We briefly present the main functionality of this package and give a few examples.
%TODO: give an url where the package can be downloaded.
...
computes the symmetric monomial function $M_{\lambda}(\vec{Y})$ for partition
$\lambda=(\lambda_{1},\ldots,\lambda_{k})$ and variables $\vec{Y}=(y_{1},\ldots,y_{m})$,
by \eqref{eq:MZonalComputation}. Note that when $m<k$, $M_{\lambda}(\vec{Y})=0$.
...
computes the coefficient~$c_{\kappa,\lambda}$ for partitions $\kappa=\left(\kappa_{1},\ldots,\kappa_{k}\right)\geq\lambda=(\lambda_{1},\ldots,\lambda_{l})$, by (\ref{eq:CRec}) and (\ref{eq:CInitial}). Note that if $\lambda>\kappa$, $c_{\kappa,\lambda}=0$. 
...
computes the zonal polynomial $\mathcal{C}_{\lambda}(\vec{Y})$ for
partition $\lambda=(\lambda_{1},\ldots,\lambda_{k})$ and variables
$\vec{Y}=(y_{1},\ldots,y_{m})$, by \eqref{eq:CInTermsOfM}.
Note that when $m<k$, $\mathcal{C}_{\lambda}(\vec{Y})=0$.
...
\begin{rem}
The Sage software has built-in functions for Jack symmetric
...
$\mathcal{Z}_{[4,2,1]}(a,b,c)$ and $\mathcal{C}_{[4,2,1]}(a,b,c)$. Moreover, this
example also shows that $c_{[4,2,1],[4,2,1]}=
\mathcal{C}_{[4,2,1]}(a,b,c)/\mathcal{Z}_{[4,2,1]}(a,b,c)=288/5$, which confirms
Conjecture \ref{conj:diag3}.
