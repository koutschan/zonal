...
%Please see Section \ref{sec:Package}, for one specific example. 
...
\begin{proof}
Obviously, 
%$\rho_{(n)}=n\left(n-1\right)$ and $\rho_{\left(n-m,m\right)}=\left(n-m\right)\left(n-m-1\right)+m\left(m-2\right)$,
%so that 
...
\end{align*}
%\begin{align*}
% & c_{(n),\left(n-m-1,m+1\right)}  \\
% =& \frac{1}{\left(m+1\right)\left(2\left(n-m\right)-1\right)}\sum_{t=1}^{m+1}\left(n-2m-2+2t\right)c_{(n),\mu}\\
%= & \frac{1}{\left(m+1\right)\left(2\left(n-m\right)-1\right)}\biggl(\left(n-2m\right)c_{(n),\left(n-m,m\right)}+\sum_{t=2}^{m+1}\left(n-2m-2+2t\right)c_{(n),\mu}\biggr)\\
% =& \frac{1}{\left(m+1\right)\left(2\left(n-m\right)-1\right)}\biggl(\left(n-2m\right)c_{(n),\left(n-m,m\right)}+\sum_{t=1}^{m}\left(n-2m+2t\right)c_{(n),\mu}\biggr)\\
% =& \frac{c_{(n),\left(n-m,m\right)}}{\left(m+1\right)\left(2\left(n-m\right)-1\right)}\Bigl(\left(n-2m\right)+m\left(2\left(n-m\right)+1\right)\Bigr) \\
...
%\begin{align*}
%  c_{(n-2,2),(n-2,2)}
%  &= \binom{n}{n-2,2}-c_{(n),(n-2,2)}-c_{(n-1,1),(n-2,2)}\\
...
%% \begin{align*}
%% c_{(a,a-b),\left(a-4,a-b+4\right)} & =\frac{1}{4\left(2b-7\right)}\left[\left(b-6\right)c_{(a,a-b),(a-3,a-b+3)}+c_{(a,a-b),(a-3,a-b+3)}\cdot3\left(2b-5\right)\right]\\
%%  & =\frac{7\left(b-3\right)}{4\left(2b-7\right)}c_{(a,a-b),(a-3,a-b+3)}\\
%%  & =\frac{7\cdot5b\left(b-1\right)\left(b-2\right)\left(b-3\right)}{4\cdot2\left(2b-1\right)\left(2b-3\right)\left(2b-5\right)\left(2b-7\right)}c_{(a,a-b),(a,a-b)}\\
%%  & =\frac{7\cdot5\cdot3b\left(b-1\right)\left(b-2\right)\left(b-3\right)}{4\cdot3\cdot2\left(2b-1\right)\left(2b-3\right)\left(2b-5\right)\left(2b-7\right)}c_{(a,a-b),(a,a-b)}
%% \end{align*}
\end{example}
...
%  \allowdisplaybreaks
  \begin{align*}
    ...
    g_2(d) &= \frac{d(2a+2d+1)}{(a-b)(2b+4d+1)} \cdot f(a,b,d)= \frac{a! \, (a-b-1)! \, \bigl(\frac12\bigr)_{\!d}}{(d-1)! \, (b+d)! \, (a-b-d)! \, \bigl(b+d+\frac12\bigr)_{\!a-b}},
  \end{align*}
