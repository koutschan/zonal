...
of~$\lambda$, exceeds the dimension of~$Y$, one recognizes that
\eqref{eq:MatrixpFq} indeed specializes to \eqref{eq:pFq} when $Y$ is a
$(1\times1)$-matrix, since for each~$n$ only the partition $\lambda=(n)$
contributes.
...
than parts in the partition~$\lambda$. It follows that $\mathcal{C}_{\kappa}(Y)=0$ if the
...
\begin{equation}\label{eq:CRec}
  c_{\kappa,\lambda} =
  \sum_{\lambda<\mu\leq\kappa}\frac{\left(\lambda_{i}+t\right)-\left(\lambda_{j}-t\right)}{\rho_{\kappa}-\rho_{\lambda}}c_{\kappa,\mu},
\end{equation}
where for $\lambda=\left(\lambda_1,\ldots,\lambda_k\right)$
the sum is over all $\mu=\left(\lambda_1,\ldots,\lambda_i+t,\ldots,\lambda_j-t,\ldots,\lambda_k\right)$
...
coefficient. More precisely, let $\lambda=\left(\lambda_1,\ldots,\lambda_k\right)\in\mathcal{P}_{n}$,
\begin{equation}
...
\begin{proof}
  ...
  \begin{align*}
    0 = \mathcal{C}_{\kappa}(y_1,\dots,y_{p-1}) &=
