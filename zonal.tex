...
this hypergeometric function to matrix arguments, as follows.
\begin{defn}
...
\begin{thm}\label{thm:zero2}
  ...
  if and only if there exists $p\in\mathbb{N}$ such that
...
so that 
\[
  \rho_{\left(n\right)}-\rho_{\left(n-m,m\right)}=m\left(2n-2m+1\right).
\]
By \eqref{eq:CRec},
\begin{align*}
  c_{\left(n\right),\left(n-m,m\right)}
...
\end{align*}
Now, we proceed by induction on~$m$. When $m=1$, $t=1$ in the sum above is equivalent to $\mu=(n)$, so that
